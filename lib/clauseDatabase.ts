...
// Legal positions and references are current as of NDI Rules 2019 / Companies Act 2013 / CCI 2025
...
export const clauseChunks: ClauseChunk[] = [
  ...
  // ── FOUNDER VESTING WITH LIQUIDITY CARVEOUT ──────────────────────
  {
    id: 'fv-carveout-001',
  ...
  },
  {
    ...
    section: 'Exit Rights',
    stage: 'Seed, Series A',
    title: 'Right of First Offer (ROFO) vs Right of First Refusal (ROFR) — Transfer Restrictions',
    content:
      'Both ROFO and ROFR require a selling shareholder to offer existing shareholders (typically the Company, then Investors pro-rata) the opportunity to purchase shares before a third-party sale — but the sequencing and price-discovery mechanics differ materially. Under a ROFO, the seller must first offer the shares to the ROFO holder, who makes an offer specifying a price. If the seller rejects that offer, the seller is free to sell to a third party, but only at a price not lower than what the ROFO holder offered, and on similar terms — so price discovery starts with the ROFO holder. Under a ROFR, the seller instead approaches third parties first; a third party undertakes diligence and makes a genuine offer; the seller must then notify the ROFR holder of that offer, and the ROFR holder can buy by matching or bettering it — so price discovery is done by the third party. Example: A and B are the only two shareholders. If B holds a ROFO over A\'s shares, A must first offer to B; only if B declines, or a third party will pay more than B offered, can A sell externally. If B holds a ROFR instead, A must first find a third-party offer; A then takes that price to B, and B can buy by matching or beating it.',
    tsPosition:
      'Specify which mechanic — ROFO or ROFR — applies, and to whose shares (Company first, then Investors pro-rata, is standard for founder/promoter share transfers). Do not leave the term sheet silent on this distinction; "right of first refusal" is often used loosely in India to mean either mechanic, and the difference has real economic consequences for whoever ends up selling.',
    founderPosition:
      'Position depends on who is more likely to be the seller. Where the right is held by Investors over Founder shares (the common case — restricting founder transfers), founders should push for ROFO rather than ROFR: ROFO gives the founder a price floor to shop around with, whereas a ROFR held by investors over founder shares discourages third parties from bidding at all, since the investor can simply match the price after the third party has done the diligence work. Where founders instead hold a right over Investor shares (less common, but arises in tighter cap tables), founders as the right-holder would prefer ROFR — it lets them wait for the market to do price discovery and simply match, rather than having to set and defend a price themselves.',
    investorPosition:
      'As the more frequent right-holder (over founder/promoter share transfers), investors will often prefer ROFR — it requires no upfront pricing effort and lets a third party\'s diligence set the benchmark, which the investor can then match or slightly beat. Where an investor is instead the seller (exiting the company) and a ROFO/ROFR is held by the promoters, the investor will strongly prefer a ROFO in favour of the promoters: it gives the investor an initial price to use as a credible floor when marketing to third parties, whereas a ROFR in favour of promoters chills third-party interest, since a diligence-heavy prospective buyer risks losing the deal to the promoter matching at the last step.',
    marketPractice:
      'ROFR (in favour of the Company, then Investors pro-rata) over founder/promoter share transfers is the more commonly used term in Indian term sheets, though ROFO appears in deals where founders have negotiating leverage or where investor-side counsel wants to avoid deterring competitive interest in the investor\'s own eventual exit. The two are frequently drafted loosely or interchangeably in India, so the term sheet should specify the actual mechanic, not just the label. A softened ROFR — requiring the right-holder to beat the third-party price by an agreed margin (not just match it) — is sometimes used to preserve third-party incentive to diligence and bid. Overall transfer restrictions to competitors typically continue to apply regardless of which mechanic is chosen.',
    marketStandard: true,
    risk: 'medium',
    keywords: ['ROFR', 'ROFO', 'right of first refusal', 'right of first offer', 'transfer restrictions', 'pre-emption', 'share transfer', 'exit', 'price discovery'],
    negotiationTips:
      'Do not default to whichever label the other side\'s draft uses — confirm the actual mechanic. If you are more likely to be the seller (e.g., founders under an investor-held right), push for ROFO: it sets a price floor and preserves third-party interest in bidding. If you are the right-holder and want maximum leverage at minimal effort, ROFR favours you. If a ROFR is unavoidable, negotiate a "beat by X%" requirement rather than a bare "match" right, to keep third parties incentivized to diligence and bid in the first place. Also confirm the carve-out list (affiliate transfers, family trusts, drag-along/tag-along exercise, Qualified IPO) at term sheet stage.',
    shaNote:
      'Full mechanics — notice periods, exercise windows, price/terms matching procedure, and treatment of partial exercise by multiple right-holders — are detailed in the SHA. Term sheet should lock which mechanic (ROFO or ROFR) applies and the exercise window (typically 30–45 days) to prevent SHA-stage drift toward whichever mechanic favours the drafting party.',
    legalFlag:
      'Under Companies Act 2013 Section 2(68), a private company\'s Articles of Association must restrict share transferability as a condition of remaining a private company — the ROFO/ROFR mechanic should be embedded in the Articles, not left as a purely contractual SHA restriction, for it to bind the Company and be enforceable on the register of members. For sectors subject to foreign investment conditionalities requiring Indian promoter/resident majority control, a drag or forced transfer of Indian promoter shares to a foreign third party may not be permissible even where contractually agreed. For cross-border transfers, pricing on exercise must comply with NDI Rules 2019 Rule 21: a transfer from a resident to a non-resident cannot be priced below Fair Market Value, and a transfer from a non-resident to a resident cannot be priced above Fair Market Value — this constrains the "match the third-party price" mechanic where the right-holder and the original third-party offeror sit on opposite sides of the residency line.',
    fallAway: false,
    tier: 'Typically Company first, then Investors pro-rata, over Founder/Promoter share transfers. Reverse arrangements (Founders/Company holding rights over Investor shares) are less common but do occur.',
  },
