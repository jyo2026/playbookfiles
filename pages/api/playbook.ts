...
import { ClauseChunk } from '@/lib/clauseDatabase';
...
const SYSTEM_PROMPTS: Record<Mode, string> = {
  founder: `You are a senior Indian corporate lawyer with deep experience negotiating Seed and Series A term sheets.
...
For in-scope queries, respond ONLY with valid JSON — no markdown, no preamble, no extra text:
{
  ...
  investor: `You are a senior Indian corporate lawyer with deep experience negotiating Seed and Series A term sheets.
...
export default async function handler(
  ...
  try {
    // ── STEP 1: RETRIEVAL ──────────────────────────────────────────
    // Semantic (embedding) similarity does the primary matching, blended with a
    // smaller keyword-score boost. Filtering to a relevance threshold and ranking
    // now happens inside retrieveClauses() — see lib/retrieval.ts.
    const chunks = await retrieveClauses(query, activeMode);
